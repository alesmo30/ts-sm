...
**Docker: `web`'s Vite dep pre-bundle cache goes stale after editing `packages/shared` while containers are already up.** `docker-compose.yml` bind-mounts `packages/shared` and `apps/web` into the `web` container, and `node_modules/@ts-sm/shared` inside it is a plain pnpm symlink back to that same bind-mounted directory — so a host-side `pnpm --filter shared build` *is* visible to the container immediately at the filesystem level. The trap is one layer up: Vite's dependency pre-bundle (`apps/web/node_modules/.vite/deps/@ts-sm_shared.js`) lives inside the `web-node-modules` **named volume**, which persists across `docker compose down`/`up` and even `--build`, and Vite only invalidates that cache when it detects a change to the dependency's `package.json` or the lockfile — not to the dependency's actual source or `dist/`. Editing a contract and rebuilding `shared` does neither, so the dev server keeps serving the old pre-bundled schema (symptom: new Zod union members get silently rejected client-side with `invalid_union_discriminator`, or a new export throws `is not exported by`, even though `dist/` on disk is correct and `pnpm --filter shared build` reported success). Fix: `docker compose exec web sh -c "rm -rf /repo/apps/web/node_modules/.vite" && docker compose restart web`. Rebuilding the `api` image (`docker compose up --build`) does **not** touch this — `api` has no such cache, but it *does* need its own image rebuilt after backend changes, since `apps/api`'s Dockerfile bakes `dist/` at build time with no bind mount.
