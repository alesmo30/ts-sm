...
## Fricciones de SPEC 06 (capa de voz)

- **Import circular entre `voice.module.ts` y `voice.controller.ts` rompía el arranque real, invisible en `pnpm test`.** El token `VOICE_CONFIG` se declaraba en `voice.module.ts` y `voice.controller.ts` lo importaba desde ahí con `@Inject(VOICE_CONFIG)`; `voice.module.ts` a su vez importa `VoiceController`. Con módulos circulares, Node resuelve el archivo que se cargó primero con sus exports aún a medias — `VOICE_CONFIG` llegaba `undefined` al decorador en tiempo de ejecución. `tsc --noEmit`, ESLint y los 69 tests de Jest (con Nest `Test.createTestingModule`, que no pasa por la resolución real de imports circulares del bundler) pasaban en verde sin detectarlo. Se manifestó recién al correr `docker compose up` limpio: `UndefinedDependencyException` al instanciar `VoiceController`. Corregido moviendo el token a un archivo propio, `voice.tokens.ts`, sin dependencias hacia controller ni module. **Lección para D2+:** un token de inyección compartido entre un módulo y algo que ese módulo importa (controller, gateway, service) va en su propio archivo desde el primer commit — el patrón de SPEC 04 (`LlmPort` como clase abstracta en `llm.port.ts`, separado de `llm.module.ts`) ya evitaba esto sin que quedara explícito como regla; acá se hace explícito.
- **Verificar el arranque real con `docker compose up` limpio es lo único que atrapa este tipo de bug.** Confirma la fricción ya anotada en SPEC 04 sobre no confiar en que "todos los tests pasan" implica "el contenedor arranca" — acá el caso concreto es una circularidad de módulos, no un problema de Dockerfile, pero la lección es la misma: correr el stack completo antes de dar un spec por cerrado.
- **Un proceso `node .../apps/api/dist/apps/api/src/main.js` de una sesión anterior seguía vivo en el puerto 3000**, heredado de una corrida manual fuera de Docker, y bloqueó `docker compose up` con `address already in use`. No es un bug del spec — es higiene de entorno de desarrollo — pero vale dejarlo anotado porque el mensaje de error de Docker no deja obvio que la causa es un proceso *fuera* de Docker.
- **El SDK `@deepgram/sdk` v5 es un cliente Fern autogenerado, muy distinto de los ejemplos v3 que suelen aparecer en tutoriales.** STT streaming vive en `client.listen.v1.connect({...}).sendMedia()/.sendCloseStream()`; TTS es REST no-streaming en `client.speak.v1.audio.generate({...})` (la variante `speak.v2` con streaming real por WebSocket es Flux, solo inglés, Early Access — no sirve para `aura-2-celeste-es`). El campo `Authorization` de `connect()` está tipado como obligatorio en el `.d.ts` aunque el propio README lo omite en sus ejemplos; se pasó explícito (`token ${apiKey}`) para no depender de ese comportamiento no documentado. La "sintetización por frase" del spec se implementó como una llamada REST completa por frase (no streaming dentro de una frase) — más simple y suficiente para el objetivo de "no esperar la respuesta completa", sin abrir un segundo tipo de conexión WebSocket hacia Deepgram solo para TTS.
- **`new URL('./archivo.ts', import.meta.url)` (con o sin `?url`) no empaqueta nada para un `AudioWorkletProcessor` en el build de producción de Vite, y falla en silencio.** `pnpm --filter web build` terminaba en verde y sin ningún warning, pero el `dist/assets/index-*.js` resultante no contenía el string `registerProcessor` en ninguna parte — el código del worklet simplemente no entraba al bundle. En dev (`vite --host`) el patrón sí "funciona", porque el navegador pide el archivo fuente directo al dev server y Vite lo transpila al vuelo; el problema es invisible hasta que se corre `vite build` de verdad, exactamente el mismo tipo de bug que SPEC 04 encontró con los Dockerfiles ("nadie había corrido X completamente en limpio"). Corregido moviendo el procesador a `apps/web/public/worklets/mic-processor.js` (JS plano, sin TS, servido tal cual por Vite fuera del pipeline de módulos) y apuntando `audioWorklet.addModule('/worklets/mic-processor.js')` a esa ruta absoluta. Verificado con `find apps/web/dist` + `grep registerProcessor` sobre el archivo emitido. **Regla para D2+:** cualquier AudioWorklet o Worker que deba sobrevivir a `vite build` va en `public/`, no se referencia con `new URL(..., import.meta.url)` sobre un `.ts` del árbol de `src/`.
